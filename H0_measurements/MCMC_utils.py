...
from modify_yCL import get_yCL
try:
    import emcee
except Exception:
...
def run_MCMC(nwalkers,chain,data_paths,lstsq_results,
             prior_width_ratio,outpath=None,contd=False,alpha=0, HF=True):
    '''
    a wrapper file
    '''
    if outpath is None:
    ...
    hubble_flow, pantheon = get_yCL(alpha = alpha, SNe_data = 'data/Pantheon+SH0ES.dat', SNe_cov = 'data/Pantheon+SH0ES_STAT+SYS.cov',
                                    Riess_Y = Y_fits_path, Riess_L = L_fits_path, Riess_C = C_fits_path)
